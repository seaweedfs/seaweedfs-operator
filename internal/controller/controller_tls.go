...
func (r *SeaweedReconciler) ensureTLS(ctx context.Context, m *seaweedv1.Seaweed) (bool, ctrl.Result, error) {
	...
	if done, res, err := r.ensureSecurityConfigMap(ctx, m); done {
		return done, res, err
	}
	return ReconcileResult(nil)
}
...
func (r *SeaweedReconciler) ensureSecurityConfigMap(ctx context.Context, m *seaweedv1.Seaweed) (bool, ctrl.Result, error) {
	...
	var jwtKey, jwtRead, jwtFilerWrite, jwtFilerRead string
	if err == nil {
		jwtKey = extractTOMLKey(existing.Data["security.toml"], "jwt.signing", "key")
		jwtRead = extractTOMLKey(existing.Data["security.toml"], "jwt.signing.read", "key")
	...
	cm := &corev1.ConfigMap{
		...
		Data: map[string]string{
			"security.toml": renderSecurityTOML(jwtKey, jwtRead, jwtFilerWrite, jwtFilerRead),
		},
	}
...
func renderSecurityTOML(jwtKey, jwtRead, jwtFilerWrite, jwtFilerRead string) string {
	return fmt.Sprintf(`# generated by seaweedfs-operator — do not edit
...
key  = "%s/tls.key"
`,
		jwtKey, jwtRead, jwtFilerWrite, jwtFilerRead,
		tlsMountPath,
